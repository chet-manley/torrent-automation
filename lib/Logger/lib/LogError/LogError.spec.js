  ...
  describe(`${NAME} Constructor`, () => {
    ...
    describe('Preset error messages', () => {
      it('should return a "default" preset', () => {
        const error = new LogError('default')
      ...
      it('should return a "level:range" preset', () => {
        ...
        const error = new LogError({preset: 'level:range', expected, received})
        expect(error.type).toBe('level:range')
      ...
      it('should return a "level:type" preset', () => {
        const type = 'test'
        const error = new LogError({preset: 'level:type', received: [type]})
        expect(error.type).toBe('level:type')
        expect(error.message).toBe(
