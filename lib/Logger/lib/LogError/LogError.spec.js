...
describe(`${NAME} Class`, () => {
  ...
  describe(`${NAME} Instance`, () => {
    ...
    describe('Preset error messages', () => {
      ...
      it('should return a "message:range" preset', () => {
        const testError = new LogError('message:range')
        expect(testError.type).toBe('message:range')
        expect(testError.message).toBe(PRESETS['message:range'])
      })
